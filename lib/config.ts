...
export class Config {
  ...
  async write(config: { [key: string]: string | number }) {
    ...
    return await fs.promises.writeFile(this.path, JSON.stringify(this.config));
  }
  ...
  static async write(path: PathLike, config: { [key: string]: string | number }) {
    ...
    return await fs.promises.writeFile(path, JSON.stringify(exconfig));
  }

  static writeSync(path: PathLike, config: { [key: string]: string | number }) {
    const exconfig = Config.readSync(path);
    Object.assign(exconfig, config);
    return fs.writeFileSync(path, JSON.stringify(exconfig));
  }
}
